...
Movie = Vars.get(SWIGEM5Sc.VarNames.MoviePower);
...
imHandle = Vars.handles.get_handle_base(SWIGEM5Sc.VarNames.imgPowerHandle);
...
[cartXLim, cartYLim, buffer] = SWIGEM5Sc.func.scanConversion(Movie(:,:,1));
...
if ishandle(primROI) && ishandle(pltROI)
    ...
    function [ax, pltDot, pltROI, imHandle] ...
        = Init(imHandle, XLIM, YLIM, Resource, P, buffer, pltDot, pltROI, ax)
        %% Initialize
        ...
        TTL = sprintf('Power Visualization with %s', P.transName);
        
        mainWinPos = Resource.DisplayWindow(1).Position;
        pos = ceil([mainWinPos(1) + 2.4 * mainWinPos(3) ...
        ...
        ax = axes(fig);
        imHandle = imagesc(ax, XLIM, YLIM, buffer, [0, 70]);
        hold(ax,'on')
        ...
        Vars.handles.set_handle_base(SWIGEM5Sc.VarNames.figPowerHandle, fig)
        Vars.handles.set_handle_base(SWIGEM5Sc.VarNames.imgPowerHandle, imHandle)
